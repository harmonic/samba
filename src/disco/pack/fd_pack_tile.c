...
poll_next_execle:
  ...
  /* Have I sent the max allowed microblocks? Nothing to do (unless past end time, so the crank below can end the slot). */
  if( FD_UNLIKELY( !past_end_time && ctx->slot_microblock_cnt>=ctx->slot_dynamic_max_microblocks ) ) return;

  /* Do I have enough transactions and/or have I waited enough time? (Past end time, don't wait.) */
#if !SMALL_MICROBLOCKS
  if( FD_UNLIKELY( !past_end_time && (ulong)(now-ctx->last_successful_insert) <
