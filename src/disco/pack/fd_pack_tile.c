...
during_housekeeping( fd_pack_ctx_t * ctx ) {
  ...
  /* This branch intentionally skips the upstream dynamic microblock
     bound update (compute_dynamic_max_microblocks).  Pack remains
     leader past slot_end_ns to drain votes, but the bound collapses to
     slot_microblock_cnt at slot end, which would starve the drain loop
     in after_credit and deadlock the slot.  It can't be restored at
     slot end either: POH requires published bounds to be monotonically
     decreasing.  Skipping it costs nothing on Frankendancer, where the
     POH hash-ahead optimization it feeds is a no-op (the pohh tile
     discards FD_PACK_MSG_REDUCE_MB_BOUND), so
     slot_dynamic_max_microblocks stays at slot_max_microblocks for the
     whole slot. */
}
...
after_credit( fd_pack_ctx_t *     ctx,
  ...
  /* If any execle are busy, check up to min(outstanding, 8) of them to
...
poll_next_execle:
    ...
    if( FD_UNLIKELY(
      ...
      *charge_busy = 1;
      ...
      if( FD_UNLIKELY( ctx->harmonic && fd_pack_harmonic_inflight_cnt( ctx->pack ) > 0UL ) ) {
        ...
        FD_LOG_NOTICE(( "HARMONIC: execle %d completed, inflight=%lu", poll_cursor, fd_pack_harmonic_inflight_cnt( ctx->pack ) ));
      } else {
    ...
    if( --polls_remaining && busy_bitset ) goto poll_next_execle;
  ...
  if( FD_UNLIKELY( past_end_time && ctx->leader_slot!=ULONG_MAX ) ) {
    ...
    /* Still have pending votes - will continue with vote-only scheduling below */
  }
  ...
  *charge_busy = 1;
  ...
  if( FD_LIKELY( ctx->execle_idle_bitset ) ) { /* Optimize for schedule */
    ...
    /* If past end time, only schedule votes to drain the vote buffer */
    ...
    if( FD_LIKELY( schedule_cnt ) ) {
      ...
      int is_ib = !!(microblock_dst->txnp->flags & FD_TXN_P_FLAGS_INITIALIZER_BUNDLE);
      ...
      FD_LOG_INFO(( "HARMONIC: scheduled %s cnt=%lu execle=%d remaining_ms=%ld slot=%lu",
  ...
  /* Did we send the maximum allowed microblocks? Then end the slot.
     >= rather than == because a bundle increments slot_microblock_cnt
     by its full transaction count and can jump past the bound. */
