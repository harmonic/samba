...
fd_bundle_client_request_failed( fd_bundle_tile_t * ctx,
  ...
  case FD_BUNDLE_CLIENT_REQ_Auth_GenerateAuthTokens:
    ...
    break;
  /* Clear the wait/live flags for any in-flight subscribe/unary request
     so the reconnect path in fd_bundle_client_step_reconnect can re-issue
     it.  Without this, an HTTP-level error (e.g. 5xx) leaves the request
     stuck in the "wait" state forever (the in-line switch in
     fd_bundle_client_grpc_rx_end is skipped on h2_status!=200). */
  case FD_BUNDLE_CLIENT_REQ_Bundle_GetBlockBuilderFeeInfo:
    ctx->builder_info_wait = 0;
    break;
  case FD_BUNDLE_CLIENT_REQ_Bundle_SubscribePackets:
...
fd_bundle_client_grpc_rx_end(
  ...
  if( FD_UNLIKELY( resp->h2_status!=200 ) ) {
    ...
    if( FD_UNLIKELY( request_ctx==FD_BUNDLE_CLIENT_REQ_SetStrategy ) ) {
      /* TODO: fail the connection here once all block engines implement SetStrategy */
      ctx->set_strategy_wait = 0;
      ctx->set_strategy_done = 1;
      FD_LOG_WARNING(( "SetStrategy returned HTTP %u; continuing without setting strategy", resp->h2_status ));
      return;
    }
  ...
  case FD_BUNDLE_CLIENT_REQ_SubmitLeaderWindowInfo:
    ...
    break;
  case FD_BUNDLE_CLIENT_REQ_SetStrategy:
    /* TODO: stop ignoring gRPC errors here once all block engines implement SetStrategy */
    ctx->set_strategy_wait = 0;
    ctx->set_strategy_done = 1;
    break;
  default:
...
# if FD_HAS_OPENSSL
  if( ctx->tpu_is_ssl ) {
    BIO * bio = fd_openssl_bio_new_socket( ctx->tpu_tcp_sock, BIO_NOCLOSE );
    if( FD_UNLIKELY( !bio ) ) {
      FD_LOG_ERR(( "fd_openssl_bio_new_socket for TPU endpoint failed" ));
    }
...
fd_bundle_tpu_client_handle_tpu_configs(
  ...
  ushort const new_tpu_port = res.has_tpu ? (ushort)res.tpu.port : (ushort)0;
  ushort const new_fwd_port = res.has_tpu_forward ? (ushort)res.tpu_forward.port : (ushort)0;

  /* Bump seq only when advertised sockets change so bundle_gossi isn't republished
     every GetTpuConfigs TTL refresh with identical data. */
  int const tuple_changed = ( !ctx->tpu_config_avail )
      || ( tpu_ip4_addr                     != ctx->tpu_config_tpu_ip4_addr     )
      || ( new_tpu_port                     != ctx->tpu_config_tpu_port         )
      || ( tpu_fwd_ip4_addr                 != ctx->tpu_config_tpu_fwd_ip4_addr )
      || ( new_fwd_port                     != ctx->tpu_config_tpu_fwd_port     );

  ctx->tpu_config_tpu_ip4_addr     = tpu_ip4_addr;
  ctx->tpu_config_tpu_port         = new_tpu_port;
  ctx->tpu_config_tpu_fwd_ip4_addr = tpu_fwd_ip4_addr;
  ctx->tpu_config_tpu_fwd_port     = new_fwd_port;
  ctx->tpu_config_avail            = 1;
  ctx->tpu_config_valid_until      = fd_bundle_now() + (long)60e9;  /* 60 second TTL */
  if( tuple_changed ) ctx->tpu_config_seq++;
...
fd_bundle_tpu_client_request_failed( fd_bundle_tile_t * ctx,
  ...
  case FD_BUNDLE_CLIENT_REQ_Auth_GenerateAuthTokens:
    ...
    break;
  /* Clear the wait/live flags for any in-flight TPU request so the
     reconnect path can re-issue it.  Required because rx_end short-
     circuits on h2_status!=200 / grpc_status!=OK before the in-line
...
fd_bundle_tpu_client_grpc_rx_end( void *                app_ctx,
  ...
  case FD_BUNDLE_CLIENT_REQ_SubscribePacketsTPU:
    ...
    fd_bundle_tpu_client_backoff( ctx, fd_bundle_now() );
    ctx->tpu_defer_reset = 1;
    FD_LOG_INFO(( "TPU SubscribePackets stream failed (gRPC status %u-%s). Reconnecting ...",
                  resp->grpc_status, fd_grpc_status_cstr( resp->grpc_status ) ));
    break;
  case FD_BUNDLE_CLIENT_REQ_GetTpuConfigs:
    ctx->tpu_config_wait = 0;
