...
fd_bundle_tile_maybe_sleep( fd_bundle_tile_t * ctx, long now_ns ) {
  // Harmonic: Keep sessions alive to enable connection monitoring
...
metrics_write( fd_bundle_tile_t * ctx ) {
  ...
  FD_MCNT_SET( BUNDLE, TPU_PKT_RX,             ctx->metrics.tpu_packet_received_cnt   );
  FD_MCNT_SET( BUNDLE, TPU_TXN_RX,             ctx->metrics.tpu_txn_received_cnt      );
  FD_MCNT_SET( BUNDLE, BLOCK_RX,               ctx->harmonic_block_received_cnt       );
  FD_MCNT_SET( BUNDLE, BLOCK_TXN_RX,           ctx->harmonic_block_txn_received_cnt   );
...
fd_bundle_tile_tpu_gossip_bucket( int tpu_st ) {
  return (uchar)( ( tpu_st==FD_BUNDLE_STATE_CONNECTED ) ? 2 : 0 );
}
...
during_frag( fd_bundle_tile_t * ctx,
  ...
  if( FD_UNLIKELY( in_idx==ctx->leader_in.idx &&
                   ( !ctx->leader_in_is_replay || sig==REPLAY_SIG_BECAME_LEADER ) ) ) {
...
after_frag( fd_bundle_tile_t *  ctx,
  ...
  // Harmonic: notify block engine on became_leader
  if( FD_UNLIKELY( in_idx==ctx->leader_in.idx &&
                   ( !ctx->leader_in_is_replay || sig==REPLAY_SIG_BECAME_LEADER ) ) ) {
    if( FD_LIKELY( !ctx->submit_leader_window_info_wait ) ) {
...
fd_bundle_tile_publish_tpu_update(
  ...
  int is_connected = ( fd_bundle_tpu_client_status( ctx ) == FD_BUNDLE_STATE_CONNECTED );
...
after_credit( fd_bundle_tile_t *  ctx,
  ...
  if( FD_UNLIKELY( ctx->gossip_out.mem && ctx->tpu_conn_enabled ) ) {
    ...
    int const cfg_changed = ( tpu_st==FD_BUNDLE_STATE_CONNECTED &&
...
unprivileged_init( fd_topo_t const *      topo,
  ...
  ctx->sleep_mode       = has_replay_in && !ctx->harmonic_block_mode; /* start asleep until we learn leader schedule; harmonic keeps connection alive */
  ctx->sleep_check_ns   = 0;
...
before_frag( fd_bundle_tile_t * ctx,
  ...
  /* Ignore messages from leader link that are not became_leader (or replay_out reset for sleep) */
  if( FD_UNLIKELY( in_idx == ctx->leader_in.idx ) ) {
    if( FD_LIKELY( ctx->leader_in_is_replay ) ) {
      /* replay_out uses REPLAY_SIG_BECAME_LEADER for harmonic and REPLAY_SIG_RESET for sleep */
      if( FD_UNLIKELY( sig != REPLAY_SIG_BECAME_LEADER && sig != REPLAY_SIG_RESET ) ) return 1; /* Discard */
    } else {
