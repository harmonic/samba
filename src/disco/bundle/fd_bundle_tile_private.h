...
struct fd_bundle_pending_txn {
  ...
  ushort payload_sz;
  uint   source_ipv4;
  uchar  source_tpu;  /* FD_TXN_M_TPU_SOURCE_BUNDLE or FD_TXN_M_TPU_SOURCE_HARMONIC */
  ulong  sig;
  ulong  bundle_seq;
...
# endif /* FD_HAS_OPENSSL */
  ...
  /* Monotonic counter bumped on each successful GetTpuConfigs decode so we
     can republish bundle_gossi when relayer IPs change without a disconnect. */
  ulong tpu_config_seq; /* bumped when GetTpuConfigs yields new addr/port tuple (see fd_bundle_tpu_client_handle_tpu_configs) */
  ulong tpu_gossip_cfg_seq; /* tpu_config_seq last published to pohh/gossip */
  ...
  /*
     ...
     frankendancer: pohh_pack (or poh_pack)
     firedancer:    replay_out
   */
  fd_bundle_out_ctx_t leader_in;
  int leader_in_is_replay; /* 1 if using replay_out, 0 if using pohh_pack/poh_pack */
  ...
  /* Last TPU gossip bucket published (0=disconnected/connecting, 2=connected).
     Init != first bucket so we publish once at startup. */
  uchar tpu_status_gossip;  /* collapsed bucket for bundle_gossi dedupe */
...
/* fd_bundle_client_status provides a "check engine light".
   ...
   Return codes are compatible with FD_BUNDLE_BLOCK_ENGINE_STATUS_{...}. */
