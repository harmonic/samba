...
fd_config_validate( fd_config_t const * config ) {
  ...
  if( FD_UNLIKELY( config->tiles.bundle.harmonic_block_mode ) ) {
    if( FD_UNLIKELY( !config->tiles.bundle.enabled ) ) {
      FD_LOG_ERR(( "`tiles.bundle.enabled` must be true when `tiles.bundle.harmonic_block_mode` is enabled" ));
    }
    if( FD_UNLIKELY( !config->tiles.bundle.url[0] ) ) {
      FD_LOG_ERR(( "`tiles.bundle.url` must be set when `tiles.bundle.harmonic_block_mode` is enabled" ));
    }
    if( FD_UNLIKELY( !config->tiles.bundle.tpu_url[0] ) ) {
