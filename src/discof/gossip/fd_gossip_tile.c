...
#define IN_KIND_BUNDLE_GOSSIP (5)
...
handle_tpu_update( fd_gossip_tile_ctx_t *             ctx,
  ...
  if( msg->status == FD_BUNDLE_TPU_UPDATE_CONNECTED ) {
    ...
    ctx->my_contact_info->sockets[ FD_CONTACT_INFO_SOCKET_TPU ].addr               = msg->tpu_ip4_addr;
    ctx->my_contact_info->sockets[ FD_CONTACT_INFO_SOCKET_TPU ].port               = tpu_port_nbo;
    ctx->my_contact_info->sockets[ FD_CONTACT_INFO_SOCKET_TPU_QUIC ].addr          = msg->tpu_ip4_addr;
    ctx->my_contact_info->sockets[ FD_CONTACT_INFO_SOCKET_TPU_QUIC ].port          = tpu_port_nbo;
    ctx->my_contact_info->sockets[ FD_CONTACT_INFO_SOCKET_TPU_VOTE ].addr          = msg->tpu_ip4_addr;
    ctx->my_contact_info->sockets[ FD_CONTACT_INFO_SOCKET_TPU_VOTE ].port          = tpu_port_nbo;
    ctx->my_contact_info->sockets[ FD_CONTACT_INFO_SOCKET_TPU_VOTE_QUIC ].addr     = msg->tpu_ip4_addr;
    ctx->my_contact_info->sockets[ FD_CONTACT_INFO_SOCKET_TPU_VOTE_QUIC ].port     = tpu_port_nbo;
    ctx->my_contact_info->sockets[ FD_CONTACT_INFO_SOCKET_TPU_FORWARDS ].addr      = msg->tpu_fwd_ip4_addr;
    ctx->my_contact_info->sockets[ FD_CONTACT_INFO_SOCKET_TPU_FORWARDS ].port      = tpu_fwd_port_nbo;
    ctx->my_contact_info->sockets[ FD_CONTACT_INFO_SOCKET_TPU_FORWARDS_QUIC ].addr = msg->tpu_fwd_ip4_addr;
    ctx->my_contact_info->sockets[ FD_CONTACT_INFO_SOCKET_TPU_FORWARDS_QUIC ].port = tpu_fwd_port_nbo;
    ...
    ctx->my_contact_info->sockets[ FD_CONTACT_INFO_SOCKET_TPU ]               = ctx->local_tpu_addr;
    ctx->my_contact_info->sockets[ FD_CONTACT_INFO_SOCKET_TPU_QUIC ]          = ctx->local_tpu_quic_addr;
    ctx->my_contact_info->sockets[ FD_CONTACT_INFO_SOCKET_TPU_VOTE ]          = ctx->local_tpu_addr;
    ctx->my_contact_info->sockets[ FD_CONTACT_INFO_SOCKET_TPU_VOTE_QUIC ]     = ctx->local_tpu_quic_addr;
    ctx->my_contact_info->sockets[ FD_CONTACT_INFO_SOCKET_TPU_FORWARDS ]      = ctx->local_tpu_fwd_addr;
    ctx->my_contact_info->sockets[ FD_CONTACT_INFO_SOCKET_TPU_FORWARDS_QUIC ] = ctx->local_tpu_fwd_quic_addr;

    FD_LOG_INFO(( "TPU disconnected: reverted to local TPU " FD_IP4_ADDR_FMT ":%u",
                  FD_IP4_ADDR_FMT_ARGS( ctx->local_tpu_addr.addr ),
...
unprivileged_init( fd_topo_t *      topo,
  ...
  *ctx->gossvf_out = out1( topo, tile, "gossip_gossv" );
  ...
  ctx->my_contact_info->version.client      = FD_CONTACT_INFO_VERSION_CLIENT_HARMONIC_FD;
  ...
  ctx->local_tpu_addr          = ctx->my_contact_info->sockets[ FD_CONTACT_INFO_SOCKET_TPU ];
  ctx->local_tpu_quic_addr     = ctx->my_contact_info->sockets[ FD_CONTACT_INFO_SOCKET_TPU_QUIC ];
  ctx->local_tpu_fwd_addr      = ctx->my_contact_info->sockets[ FD_CONTACT_INFO_SOCKET_TPU_FORWARDS ];
  ctx->local_tpu_fwd_quic_addr = ctx->my_contact_info->sockets[ FD_CONTACT_INFO_SOCKET_TPU_FORWARDS_QUIC ];
